...
PROBLEMS:
----------
ADDITIONAL FEATURES: 1.GRADIENT BACKGROUND
                     2.RAINBOW THAT APPEARS ONLY WHEN MODE IS AUTONOMOUS
					 3.STAR APPEARS ON RIGHT CLICK AND DISSAPEARS ON RIGHT CLICK
					 4.JAM CHANGES BETWEEN FOUR COLORS WHEN F10 IS PRESSED
*********/
...
#define TIMER_PERIOD  30 // Period for the timer. 60
...
int x1 = 0, x2 = 0, x3 = 0, xA = 0, yA = 0, xStar = 0, yStar = 0;
...
0, 0, 204 ,
178, 102, 255 };
int jamColor[4][3] = { 244, 171, 200, //jam colors
236, 240, 210,
172, 246, 251,
207, 172, 251
}, jamIndex = 0;
...
bool star = false; //only appears if star is true
...
void drawStar() {
	if (star) {
		glColor3ub(223, 240, 242);
		glBegin(GL_TRIANGLES);
		glVertex2d(-30 + xStar, 0 + yStar);
		glVertex2d(-5 + xStar, 5 + yStar);
		glVertex2d(-5 + xStar, -5 + yStar);

		glVertex2d(-5 + xStar, 5 + yStar);
		glVertex2d(0 + xStar, 30 + yStar);
		glVertex2d(5 + xStar, 5 + yStar);
		...
		glVertex2d(-5 + xStar, -5 + yStar);
		glEnd();
		glRectf(-5 + xStar, -5 + yStar, 5 + xStar, 5 + yStar);
		//light effects
		glColor3f(1, 1, 1);
		glLineWidth(2);
		glBegin(GL_LINES);
		glVertex2d(1 + xStar, 25 + yStar);
		glVertex2d(4 + xStar, 2 + yStar);
		glVertex2d(4 + xStar, 2 + yStar);
		glVertex2d(-4 + xStar, -2 + yStar);
		glVertex2d(-4 + xStar, -2 + yStar);
		glVertex2d(-1 + xStar, -25 + yStar);
...
void display() {
	...
	glColor3f(1, 1, 1);
	vprint(-200, -250, GLUT_BITMAP_9_BY_15, "Press <F10> to switch between jam colors.");
	vprint(-290, -270, GLUT_BITMAP_9_BY_15, "Press <F1> to switch the mode and Arrow Keys to control the Cat.");
	vprint(-270, -290, GLUT_BITMAP_9_BY_15, "Press Right Click Button to make star appear / disappear.");
	glColor3f(0, 1, 1);
	vprint(-200, -250, GLUT_BITMAP_9_BY_15, "      <F10>");
	vprint(-270, -290, GLUT_BITMAP_9_BY_15, "      Right Click Button");
	...
	if (mode == COMPLEX) {
		...
		//body(jam)
		glColor3ub(jamColor[jamIndex][0], jamColor[jamIndex][1], jamColor[jamIndex][2]);
...
void onSpecialKeyDown(int key, int x, int y)
{
	...
	{
		switch (jamIndex) {
		case 0: jamIndex = 1; break;
		case 1: jamIndex = 2; break;
		case 2: jamIndex = 3; break;
		case 3: jamIndex = 0; break;
		}
	}
...
void onTimer(int v) {
	...
	if (activeTimer)
	{
		...
		//IMPORTANT: I DID NOT ACCOUNT THE RAINBOW AS PART OF MY COMPLEX SHAPE FOR THE DESICION OF BOUNCING WHEN THE SHAPE REACHES THE LEFT BORDER
